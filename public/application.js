...
var isBoardFull = function isBoardFull(gameArray) {
...
var onBoxClick = function onBoxClick(event) {
  ...
  var boxId = '#' + this.id;
  var idForUpdate = event.target.id;
...
var onUpdateGame = function onUpdateGame(boxId) {
  // console.log(store)
  ...
  if (playerToken === 'O') {
    ...
    var dataObject = JSON.stringify(updateX);
    api.updateGame(dataObject).then(ui.updateGameSuccess).catch(ui.updateGameFailure);
    ...
    var _dataObject = JSON.stringify(updateO);
    api.updateGame(_dataObject).then(ui.updateGameSuccess).catch(ui.updateGameFailure);
  }
};
